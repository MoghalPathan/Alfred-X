...
class MainWindow(QWidget):
    ...
    def __init__(self,parent=None):
        ...
        if self.session_id:
            for msg in chat_db.get_messages(self.session_id):
                ...
                content=self._sanitize_assistant_text(msg["content"]) if msg["role"]=="assistant" else msg["content"]
                self.add_message(name,content,is_alfred=(msg["role"]=="assistant"))
        else:
    ...
    def _process_input(self,text):
        ...
        if checked is None:
            return  # precheck handled it; _deliver_response already cleaned up _is_processing
        text = checked
        if not text:
            return
    ...
    def _sanitize_assistant_text(self, text: str) -> str:
        if not text:
            return ""
        t = str(text)
        # Round 1: [REMEMBER: ...] blocks — DOTALL covers multiline tags
        t = re.sub(r"\[\s*REMEMBER\s*:[\s\S]*?\]", "", t, flags=re.IGNORECASE)
        # Round 2: REMEMBER: on its own line (no brackets)
        t = re.sub(r"^\s*REMEMBER\s*:.*$", "", t, flags=re.IGNORECASE | re.MULTILINE)
        # Round 3: inline REMEMBER: tag with optional trailing ]
        t = re.sub(r"\bREMEMBER\s*:[^\n\]]*\]?", "", t, flags=re.IGNORECASE)
        # Round 4: REMEMBER(...) blocks
        t = re.sub(r"\bREMEMBER\s*\([^\)]*\)", "", t, flags=re.IGNORECASE)
        # Round 5: [key:value] tags the AI uses without REMEMBER prefix
        #          e.g. [interest:Karl_Marx], [topic:Philosophy]
        t = re.sub(r"\[[a-zA-Z_]\w*:[^\[\]\n]+\]", "", t)
        # Round 6: leftover key:value] fragments — THE CORE BUG
        #          When [REMEMBER: is partially stripped, "interest:Karl_Marx]" survives.
        #          This catches: word:Word_With_Underscores] patterns.
        t = re.sub(r"\b[a-zA-Z_]\w*:[\w][\w_]*\]", "", t)
        # Round 7: any remaining orphaned ] not part of markdown links [text](url)
        t = re.sub(r"\](?!\()", "", t)
        # Round 8: clean up extra blank lines
        t = re.sub(r"\n{3,}", "\n\n", t).strip()
        if not t:
